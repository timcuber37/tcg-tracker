...
public class PokeWalletClient {
    ...
    // Sync calls retry on HTTP 429 so a brushed rate limit doesn't silently drop
    // whole sets; one-off web calls (getCard) use 0 retries to fail fast.
    private static final int SYNC_RATE_LIMIT_RETRIES = 4;

    private JsonNode get(String path, Map<String, Object> query) {
        return get(path, query, 0);
    }

    private JsonNode get(String path, Map<String, Object> query, int retries) {
        try {
            ...
            if (status == 429 && retries > 0) {
                long waitSec = retryAfterSeconds(e);
                log.warn("PokéWallet rate limit on {} — waiting {}s, then retrying ({} left)", path, waitSec, retries);
                sleep(waitSec * 1000);
                return get(path, query, retries - 1);
            }
            if (status == 429) {
                log.warn("PokéWallet rate limit on {} — giving up after retries", path);
            } else if (status != 404) {
    ...
    /** Honor a Retry-After header (seconds) if present, else back off ~a minute. */
    private long retryAfterSeconds(RestClientResponseException e) {
        try {
            ...
            if (ra != null) {
                return Math.min(Long.parseLong(ra.trim()), 120);
            }
    ...
    public List<JsonNode> getAllSets() {
        JsonNode data = get("/sets", Map.of(), SYNC_RATE_LIMIT_RETRIES);
    ...
    public JsonNode getSetCards(String setId, int page, int limit) {
        return get("/sets/" + setId, Map.of("page", page, "limit", limit), SYNC_RATE_LIMIT_RETRIES);
    }
